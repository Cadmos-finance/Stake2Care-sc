...
async function main() {
    ...
    const impactVault = await ImpactVaultFactory.deploy(await testStETH.getAddress(),"MSF-Staked Ether", "MSF-STETH");
    console.log("impactVault deployed to:", await impactVault.getAddress());
    //await impactVault.deployed();

    const LidoImpactVaultDepositorFactory = await ethers.getContractFactory("LidoImpactVaultDepositor");
    const lidoImpactVaultDepositor = await LidoImpactVaultDepositorFactory.deploy(await deployer.getAddress(),await impactVault.getAddress());
    // await lidoImpactVaultDepositor.deployed();
    console.log("lidoImpactVaultDepositor deployed to:", await lidoImpactVaultDepositor.getAddress());

    await testStETH.connect(deployer).mint(deployerAddress, ethers.parseEther("10000"));
