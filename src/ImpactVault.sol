...
contract ImpactVault is ERC4626 , Ownable2Step, IImpactVault {
    ...
    */
     ...
     */
    constructor(IERC20 asset_, string memory name, string memory symbol) ERC20(name, symbol) ERC4626(asset_) {}
     ...
     */
    function _convertToSharesCompute(uint256 assets, Math.Rounding rounding, uint256 totalAssets_, uint256 totalSupply_) internal pure returns (uint256) {
        if (totalAssets_ < totalSupply_) { // if 1 Vault Share is worth less than 1 asset, obtain more shares pro-rata the vault NAV. Necessary to ensure no loss on withdrawal
     ...
     * @dev Internal conversion function (from assets to shares) with support for rounding direction.
     */
    function _convertToAssetsCompute(uint256 shares, Math.Rounding rounding, uint256 totalAssets_, uint256 totalSupply_) internal pure returns (uint256) {
        if (totalAssets_ < totalSupply_) { // if 1 Vault Share is worth less than 1 asset, obtain more shares pro-rata the vault NAV. Necessary to ensure no loss on withdrawal
    ...
    function _convertToShares(uint256 assets, Math.Rounding rounding) internal view override(ERC4626) returns (uint256) {
        ...
        return _convertToSharesCompute(assets, rounding, totalAssets_, totalSupply_);
    }
    ...
    function _convertToAssets(uint256 shares, Math.Rounding rounding) internal view override(ERC4626)  returns (uint256) {
        uint256 totalAssets_ = totalAssets();
        uint256 totalSupply_ = totalSupply();
        return _convertToAssetsCompute(shares, rounding, totalAssets_, totalSupply_);
    }

    /// @dev Calls CollectDonations and returns result of _convertToShares
    function _collectDonationsAndConvertToShares(uint256 assets, Math.Rounding rounding) internal returns (uint256) {
    ...
    function collectDonationsAndConvertToAssets(uint256 shares, Math.Rounding rounding) internal returns (uint256) {
        (, uint256 totalAssets_, uint256 totalSupply_) = collectDonations(0);
        return _convertToAssetsCompute(shares, rounding, totalAssets_, totalSupply_);
    }
    ...
    function deposit(uint256 assets, address receiver) public override(ERC4626) returns (uint256) {
        uint256 shares = _collectDonationsAndConvertToShares(assets, Math.Rounding.Down);
    ...
    function mint(uint256 shares, address receiver) public override(ERC4626) returns (uint256) {
        uint256 assets =  collectDonationsAndConvertToAssets(shares, Math.Rounding.Up);
        _deposit(_msgSender(), receiver, assets, shares);
        return assets;
    }
    ...
    function withdraw(uint256 assets, address receiver, address owner) public override(ERC4626) returns (uint256) {
        uint256 shares = _collectDonationsAndConvertToShares(assets, Math.Rounding.Up);
    ...
    function redeem(uint256 shares, address receiver, address owner) public override(ERC4626) returns (uint256) {
        uint256 assets = collectDonationsAndConvertToAssets(shares, Math.Rounding.Down);
        _withdraw(_msgSender(), receiver, owner, assets, shares);
        return assets;
    }
    ...
    function collectDonations(uint64 minimalTransfer) public override(IImpactVault) returns (uint128 collectedAmount, uint256 totalAssets_, uint256 totalSupply_){
        totalSupply_ = totalSupply();
        totalAssets_ = totalAssets();
        TimelockedSurplus memory timeLockedSurplus_ = timeLockedSurplus;
        minimalTransfer = minimalTransfer == 0 ? timeLockedSurplus_.minimalCollectAmount : minimalTransfer;
        if (totalAssets_ > totalSupply_ + minimalTransfer) { //Check if current surplus is high enough
            if (timeLockedSurplus_.timestamp < uint64(block.timestamp - 1 days)) { // 1 day TimeLock on surplus distribution - to avoid donor loss in case of potential NAV up-down bounce
                uint128 newSurplus = uint128(totalAssets_ - totalSupply_);
                collectedAmount = newSurplus > timeLockedSurplus_.surplus ? timeLockedSurplus_.surplus : newSurplus;
                ...
                timeLockedSurplus = TimelockedSurplus(newSurplus-collectedAmount,uint64(block.timestamp),timeLockedSurplus_.minimalCollectAmount);
    ...
    function setAutoCollectThreshold(uint64 newMinimalCollectAmount) external override(IImpactVault) onlyOwner {
        TimelockedSurplus memory timeLockedSurplus_ = timeLockedSurplus;
        timeLockedSurplus = TimelockedSurplus(timeLockedSurplus_.surplus,timeLockedSurplus_.timestamp,newMinimalCollectAmount);
        emit SetAutoCollectThreshold(newMinimalCollectAmount, timeLockedSurplus_.minimalCollectAmount);
    }
