...
abstract contract ImpactVaultDepositor is IImpactVaultDepositor {
    ...
    uint256 private constant maxUint = 0xffffffffffffffffffffffffffffffffffffffffffffffffffffffffffffffff;
    ...
    function _deposit(
        uint256 depositAmount) internal returns (uint256 sharesCreated){
            sharesCreated =  IERC4626(ImpactVault).deposit(depositAmount, msg.sender);
        }

    function _convertToken(uint256 amount, address token) internal virtual returns (uint256 assetAmount);
    ...
    function depositAsset(uint256 depositAmount) external override(IImpactVaultDepositor) returns (uint256 sharesCreated){
    ...
    function depositToken(uint256 amount, address token, uint256 depositProportion) external override(IImpactVaultDepositor) returns (uint256 sharesCreated, uint256 assetReturned){
    ...
    /// @param depositProportion Proportion of obtained asset to deposit into the impact vault (in wad). Remainder is sent back to msg.sender
    function depositETH(uint256 depositProportion) public payable override(IImpactVaultDepositor) returns (uint256 sharesCreated, uint256 assetReturned){
